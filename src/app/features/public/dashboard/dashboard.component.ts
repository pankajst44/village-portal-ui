...
export class DashboardComponent implements OnInit {
  ...
  stats: DashboardStats = {
    totalFundsReceived: 0, totalProjects: 0,
    ongoingProjects: 0, completedProjects: 0, plannedProjects: 0,
    totalAllocatedBudget: 0, totalSpent: 0, budgetUtilizationPct: 0,
  };
  ...
  private statusColors: Record<string, string> = {
    ONGOING: '#2e7d32', COMPLETED: '#4a148c',
    PLANNED: '#1565c0', ON_HOLD: '#e65100', CANCELLED: '#b71c1c',
  };
  ...
  private loadDashboard(): void {
    ...
    forkJoin({
      ...
      this.activeFunds    = (funds?.data as Fund[]) ?? [];
      ...
      this.projectStatusCounts = statusList
        .map(s => ({ status: s, count: allProjects.filter(p => p.status === s).length, color: this.statusColors[s] }))
        .filter(s => s.count > 0);
